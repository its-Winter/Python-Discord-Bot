...
class Audio(commands.Cog):
      def __init__(self, bot):
            ...
            if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
                  ...
                  bot.lavalink.add_node('127.0.0.1', 2333, 'youshallnotpass', 'na', 'default-node')  # Host, Port, Password, Region, Name
      ...
      @commands.guild_only()
      async def _volume(self, ctx, volume: Optional[Union[int, float]] = None):
            player = self.bot.lavalink.player_manager.get(ctx.guild.id)
            if not volume:
